...
```{r fullTree, anchor = "Figure", fig.align="center", echo=FALSE, message=FALSE, fig.cap="Phylogeny of 16S rRNA gene copy database used in this study. Long branch length indicate potential sequences in the database that were incorrectly identified as 16S rRNA genes by rnammer.", fig.height=4,fig.width=7}
tree <- read.tree("infernal_raxml_tree/RAxML_result.infernal_16S_tree")
plot.phylo(tree, type="fan" ,show.tip.label = FALSE, open.angle = 120)
```
...
```{r clusterSize, anchor = "figure", fig.align="center", echo=FALSE, message=FALSE, fig.cap="Distribution of cluster size, number of sequences assigned to a cluster, by clustering threshold.", fig.height=3,fig.width=5}
...
An alternative solution is to approach taxon estimation through the lens of expectation maximation, similar to an approach used for RNA-Seq quantification[@Patro2014]. OTUs may be collapsed into OTU equivalence classes, consisting of taxa mapping to an OTU sequence which have similar copy number of 16S sequences clustered to that OTU. 
