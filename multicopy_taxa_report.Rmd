...
A database of 16S rRNA gene copy sequences was generated by extracting 16S rRNA gene sequences from all closed bacterial genomes in the NCBI RefSeq database.  Number of 16S rRNA gene copies per genome ranged from `r min(copy_count$copies)` to `r max(copy_count$copies)` with a median of `r median(copy_count$copies)`. The distribution of 16S rRNA gene copy numbers agrees with previous studies [@Angley,@Vetrovsky], exluding a genome with 16S rRNA gene copies. Only a single method was used to identify 16S rRNA gene copy sequences and therefore may contain sequences that were incorrectly identified as 16S rRNA genes. Therefore the results below are presented as a proof of concept. A more rigorus approach to generating the sequence copy database similar to that used in Angly et al. (2014) would help validate the database. 

```{r echo=FALSE, message=FALSE, fig.cap="Distribution in the number of gene copies present in a bacterial genome."}
...
```{r echo=FALSE, message=FALSE, fig.cap="Phylogeny of 16S rRNA gene copy database used in this study."}
tree <- read.tree("infernal_raxml_tree/RAxML_result.infernal_16S_tree")
ggtree(tree, layout="circular")
```

### Clustering
The extracted 16S rRNA genes were clustered with dnaclust using a range of threshold values from 1.00 (identical gene sequences) to 0.61 (sequences with 61% similarity). For lower clustering (< 0.73) thresholds the majority of the sequences were assigned to a single cluster.

```{r echo = FALSE}
cluster_df_size <- cluster_df %>% group_by(threshold, id) %>% summarise(size = n()) 
```

```{r echo=FALSE, message=FALSE, fig.cap="Distribution of cluster size, number of sequences assigned to a cluster, by clustering threshold."}
...
### Cluster Copy Number 
The number of 16S rRNA gene copies from a single genome within a cluster varies. The variabily in copy number within a cluster biases copy number correction when only a single value in copy number correction.  Single cluster copy number value is currently the only method used in copy number correction [@Kembel2012,@Angly2014,@Stoddard,@Perisin].  CopyRighter [@Angly2014], attempts to address this issue by defining the cluster copy number correction value proportionally based on the copy number of the species assigned to a cluster.  
...
```{r echo=FALSE, message=FALSE, fig.cap="Proportion of genomes assigned to multiple clusters by clustering threshold."}
...
Taxonomic for 97% clustering threshold, >5% cluster classification error rate for genus and species level classifications. For clustering thresholds less than 0.97, the proportion of taxonomically ambiguous clusters is greater than `r min_ambig_lt97`. When analyzing 16S rRNA metagenomic datasets, the presence of taxonomically ambiguous clusters must be taken into consideration. 

```{r echo=FALSE, message=FALSE, fig.cap="Proportion of cluster with more than one assigned taxa by clustering threshold and taxonomic level."}
anno_count_fig_df %>%  
